(function(){
  ...
  const cache={};
  async function details(name){if(cache[name])return cache[name];try{const r=await fetch(API+encodeURIComponent(name));const d=await r.json();let stats=d.stats.map(x=>x.base_stat);const modified=window.MODIFIED_STATS?.[String(d.id)];if(modified)stats=modified;const out={name:pretty(d.name),id:d.id,slug:d.name,type:d.types.map(x=>pretty(x.type.name)).join(' / '),abilities:d.abilities.map(x=>x.ability.name),stats};cache[name]=out;return out}catch{return null}}
  window.POKEMON_RUNTIME={details,cache};
  fetch(LIST).then(r=>r.json()).then(async data=>{
    const opts=[];
    for(const m of data.results){const id=Number(m.url.split('/').filter(Boolean).pop());if((id>=1&&id<=721)||MEGAS.has(m.name)){const display=pretty(m.name);const base=MEGAS.has(m.name)?DEX[m.name.split('-')[0]]:id;opts.push({name:display,id:base||id,slug:m.name});}}
    window.POKEMON_DATA=opts;if(typeof render==='function')render();
  });
  document.addEventListener('change',async e=>{if(e.target?.id!=='pokeName')return;const typed=e.target.value.trim();const opt=window.POKEMON_DATA.find(x=>x.name===typed);if(!opt)return;const d=await details(opt.slug);if(!d)return;d.id=opt.id;if(window.MODIFIED_STATS?.[String(opt.id)])d.stats=window.MODIFIED_STATS[String(opt.id)];const idx=window.POKEMON_DATA.findIndex(x=>x.name===typed);if(idx>=0)window.POKEMON_DATA[idx]=d;const s=document.getElementById('pokeAbility');if(s)s.innerHTML='<option value="">Selecciona habilidad</option>'+d.abilities.map(a=>`<option>${a}</option>`).join('');});
})();
