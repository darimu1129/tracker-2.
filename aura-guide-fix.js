(function(){
  ...
  if(changed&&!sessionStorage.getItem('aura-guide-fix-reloaded')){sessionStorage.setItem('aura-guide-fix-reloaded','1');location.reload();return}
  function paint(){
    const b=window.currentBoss?.();if(!b)return;const content=document.querySelector('.content');if(!content)return;
    if(b.variants&&!content.querySelector('.aura-variants')){
      const box=document.createElement('div');box.className='card aura-variants';box.innerHTML='<strong>Equipo de Aura</strong><div class="aura-choice"><button class="btn btn-primary" data-aura-choice="fire">🔥 Fuego</button><button class="btn btn-secondary" data-aura-choice="water">💧 Agua</button><button class="btn btn-secondary" data-aura-choice="leaf">🍃 Hoja</button></div>';content.insertBefore(box,content.firstChild);
    ...
    if(document.querySelector('.crumb')?.textContent?.includes('›')&&!document.getElementById('guide')&&window.buildGuide){window.buildGuide().then(html=>{const c=document.querySelector('.content');if(c&&!document.getElementById('guide')&&html)c.insertAdjacentHTML('beforeend',html)})}
  }
  const obs=new MutationObserver(()=>setTimeout(paint,0));if(document.body)obs.observe(document.body,{childList:true,subtree:true});window.addEventListener('load',()=>setTimeout(paint,100));setTimeout(paint,300);
})();
